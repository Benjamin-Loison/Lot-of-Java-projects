...
import com.degraduck.minefus.models.items.ModelCapeRoyal;
...
public class StuffCapeBouftouRoyal extends ItemArmor
{
    private ModelBase value;

    public StuffCapeBouftouRoyal(ItemArmor.ArmorMaterial material, int type)
    {
    ...
    {
        if(slot == 1)
            return "moddofus:textures/models/armor/stuff_cape_bouftou_royal.png";
    ...
    {
        if(stack.getItem() == Dofus.StuffCapeBouftouRoyal)
        {
            return new ModelCapeRoyal(armor);
        }
        return null;
    }
    ...
    {
        par3List.add("§7Â§oCette cape portÃ©e par le grand Bouftou");
        par3List.add("§7Â§oRoyal confÃ¨re style fabuleux.");
        par3List.add("");
