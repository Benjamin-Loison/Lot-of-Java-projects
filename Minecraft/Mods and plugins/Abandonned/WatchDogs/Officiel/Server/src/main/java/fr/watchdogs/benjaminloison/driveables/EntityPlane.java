...
import fr.watchdogs.benjaminloison.packets.PacketDriveableControl;
import fr.watchdogs.benjaminloison.packets.PacketDriveableKey;
import fr.watchdogs.benjaminloison.packets.PacketPlaneControl;
...
public class EntityPlane extends EntityDriveable
{
    ...
    public EntityPlane(World world, double x, double y, double z, int v, PlaneType type, DriveableData data, String s)
    {
    ...
    public boolean pressKey(int key, EntityPlayer player)
    {
        ...
        {
            WatchDogs.getPacketHandler().sendToServer(new PacketDriveableKey(key));
            return true;
        }
        ...
        switch(key)
        {
            ...
            case 13: // Gear
            {
                if(toggleTimer <= 0)
                {
                    ...
                    toggleTimer = 10;
                    WatchDogs.getPacketHandler().sendToServer(new PacketDriveableControl(this));
                }
                return true;
            }
            case 14: // Door
    ...
    public void onUpdate()
    {
        ...
        boolean thePlayerIsDrivingThis = worldObj.isRemote && seats[0] != null && seats[0].riddenByEntity instanceof EntityPlayer;

        // Despawning
        ticksSinceUsed++;
        if(!worldObj.isRemote && seats[0].riddenByEntity != null)
            ticksSinceUsed = 0;
        if(FileAPI.configNumber("life.plane") > 0 && ticksSinceUsed > FileAPI.configNumber("life.plane") * 20)
            setDead();
        ...
        {
            PacketPlaySound.sendSoundPacket(posX, posY, posZ, WatchDogs.soundRange, dimension, type.startSound, false);
            soundPosition = type.startSoundLength;
        }
        // Flying sound
        if(throttle > 0.2F && soundPosition == 0 && hasEnoughFuel())
        {
            PacketPlaySound.sendSoundPacket(posX, posY, posZ, WatchDogs.soundRange, dimension, type.engineSound, false);
        ...
        {
            WatchDogs.getPacketHandler().sendToServer(new PacketPlaneControl(this));
            serverPosX = posX;
            serverPosY = posY;
            serverPosZ = posZ;
        ...
        {
            WatchDogs.getPacketHandler().sendToAllAround(new PacketPlaneControl(this), posX, posY, posZ, WatchDogs.driveableUpdateRange, dimension);
        }
    }
