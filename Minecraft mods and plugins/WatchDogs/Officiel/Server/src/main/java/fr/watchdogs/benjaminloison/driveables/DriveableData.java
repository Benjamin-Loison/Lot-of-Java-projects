...
public class DriveableData implements IInventory
{
    ...
    {
        if(i < getBombInventoryStart() && itemstack != null && itemstack.getItem() instanceof ItemBullet)
            return true;
        if(i >= getBombInventoryStart() && i < getMissileInventoryStart() && itemstack != null && itemstack.getItem() instanceof ItemBullet)
            return true;
        ...
        if(i >= getCargoInventoryStart() && i < getFuelSlot())
            return true;
        if(i == getFuelSlot() && itemstack != null && itemstack.getItem() instanceof ItemPart && ((ItemPart)itemstack.getItem()).type.category == 9)
            return true;
        return false;
    }
}