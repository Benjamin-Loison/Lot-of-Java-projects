...
public class Matrix3f
{
    ...
    {
        float sn = MathHelper.sin(r), cs = MathHelper.cos(r);
    ...
    {
        float[] retMat[] = new float[3][3], retVec = new float[3];
        for(int i = 0; i < 3; i++)
        {
            float[] row = {m.matrix[i][0], m.matrix[i][1], m.matrix[i][2]}, column = {(float)vec.xCoord, (float)vec.yCoord, (float)vec.zCoord};
