...
public class ItemBlockMStoneSlab extends ItemBlock
{
    private final boolean isFullBlock;
    private final Block theHalfSlab, doubleSlab;

    public ItemBlockMStoneSlab(Block block)
    {
        super(block);
        theHalfSlab = M_BlockSlab.SingleSlabMStone;
        doubleSlab = M_BlockSlab.DoubleSlabMStone;
        if(block == M_BlockSlab.DoubleSlabMStone)
            isFullBlock = true;
        else
            isFullBlock = false;
        setMaxDamage(0);
    ...
    {
        return ((BlockSlabMStone)theHalfSlab).func_150002_b(stack.getItemDamage());
    }
