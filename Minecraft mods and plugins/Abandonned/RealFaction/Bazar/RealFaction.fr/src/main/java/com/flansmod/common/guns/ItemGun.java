...
public class ItemGun extends Item implements IFlanItem {
	...
	/**
	 * Get the bullet item stack stored in the gun's NBT data (the loaded
	 * magazine / bullets)
	 */
	...
	/**
	 * Set the bullet item stack stored in the gun's NBT data (the loaded
	 * magazine / bullets)
	 */
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List lines, boolean advancedTooltips) {
		...
		for (int i = 0; i < type.numAmmoItemsInGun; i++) {
			...
			if (bulletStack != null && bulletStack.getItem() instanceof ItemBullet) {
				...
				// String line = bulletType.name + (bulletStack.getMaxDamage()
				// == 1 ? "" : " " + (bulletStack.getMaxDamage() -
	...
	public void onUpdateClient(ItemStack itemstack, World world, Entity entity, int i, boolean flag) {
		if (entity instanceof EntityPlayer && ((EntityPlayer) entity).inventory.getCurrentItem() == itemstack) {
			...
			if (soundDelay <= 0 && type.idleSound != null) {
				PacketPlaySound.sendSoundPacket(entity.posX, entity.posY, entity.posZ, FlansMod.soundRange,
						entity.dimension, type.idleSound, false);
			...
			else if (mc.objectMouseOver != null && (mc.objectMouseOver.entityHit instanceof EntityFlagpole
					|| mc.objectMouseOver.entityHit instanceof EntityFlag
					|| mc.objectMouseOver.entityHit instanceof EntityGunItem
					|| (mc.objectMouseOver.entityHit instanceof EntityGrenade
			...
			// Else do shoot code
					...
					if (data.offHandGunSlot != 0) {
						...
						if (offHandGunStack != null && offHandGunStack.getItem() instanceof ItemGun) {
							...
							if (offHandGunType.usableByPlayers) {
								// If we are using a burst mode gun, and there
								// is burst left to be done, try to do it
								if (offHandGunType.getFireMode(offHandGunStack) == EnumFireMode.BURST
									...
									// Send packet when firing a semi or
									// starting to fire a full
									...
									if ((offHandGunType.getFireMode(offHandGunStack) == EnumFireMode.FULLAUTO
											|| offHandGunType.getFireMode(offHandGunStack) == EnumFireMode.MINIGUN)
											&& !leftMouseHeld && lastLeftMouseHeld) // Full
																					// auto.
																					// Send
																					// released
																					// mouse
				...
				// --------------------------------- Main hand item
				// ---------------------------------------------
				// If we are using a burst mode gun, and there is burst left to
				// be done, try to do it
				if (type.usableByPlayers) {
					if (type.getFireMode(itemstack) == EnumFireMode.BURST && data.burstRoundsRemainingRight > 0) {
						...
						// Send packet when firing a semi or starting to fire a
						// full
						...
						if ((type.getFireMode(itemstack) == EnumFireMode.FULLAUTO
								|| type.getFireMode(itemstack) == EnumFireMode.MINIGUN) && !rightMouseHeld
								&& lastRightMouseHeld) // Full auto. Send
														// released mouse packet
						{
				...
				if (!offHandFull
						&& (type.secondaryFunction == EnumSecondaryFunction.ADS_ZOOM
								|| type.secondaryFunction == EnumSecondaryFunction.ZOOM)
						&& Mouse.isButtonDown(0) && FlansModClient.scopeTime <= 0
	...
	public void onUpdateServer(ItemStack itemstack, World world, Entity entity, int i, boolean flag) {
		if (entity instanceof EntityPlayerMP) {
			...
			if (player.inventory.getCurrentItem() != itemstack) {
				// If the player is no longer holding a gun, emulate a release
				// of the shoot button
			...
			if (data.isShootingRight) {
				...
				if (type.useLoopingSounds && data.loopedSoundDelay <= 0 && data.minigunSpeed > 0.1F
						&& !data.reloadingRight) {
					data.loopedSoundDelay = data.shouldPlayWarmupSound ? type.warmupSoundLength
							: type.loopedSoundLength;
					PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, FlansMod.soundRange,
							player.dimension, data.shouldPlayWarmupSound ? type.warmupSound : type.loopedSound, false);
				...
				if (type.useLoopingSounds && data.shouldPlayCooldownSound) {
					PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, FlansMod.soundRange,
							player.dimension, type.cooldownSound, false);
			...
			if (type.oneHanded && data.offHandGunSlot != 0) {
				...
				if (offHandGunStack != null && offHandGunStack.getItem() instanceof ItemGun) {
					...
					if (offHandGunType.getFireMode(offHandGunStack) == EnumFireMode.BURST
							&& data.burstRoundsRemainingLeft > 0) {
						player.inventory.setInventorySlotContents(data.offHandGunSlot - 1,
					...
					if (data.isShootingLeft) {
						...
						// Play looping sounds for minigun
						if (offHandGunType.useLoopingSounds && data.loopedSoundDelay <= 0 && data.minigunSpeed > 0.1F
								&& !data.reloadingLeft) {
							data.loopedSoundDelay = data.shouldPlayWarmupSound ? offHandGunType.warmupSoundLength
									: offHandGunType.loopedSoundLength;
							PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, FlansMod.soundRange,
									player.dimension, data.shouldPlayWarmupSound ? offHandGunType.warmupSound
											: offHandGunType.loopedSound,
									false);
							data.shouldPlayWarmupSound = false;
						}
						// Minigun is sufficiently fast to shoot
						if (offHandGunType.getFireMode(offHandGunStack) == EnumFireMode.MINIGUN
								&& data.minigunSpeed > offHandGunType.minigunStartSpeed)
							player.inventory.setInventorySlotContents(data.offHandGunSlot - 1,
									tryToShoot(offHandGunStack, offHandGunType, world, player, true));
					} else {
						if (offHandGunType.useLoopingSounds && data.shouldPlayCooldownSound) {
							PacketPlaySound.sendSoundPacket(player.posX, player.posY, player.posZ, FlansMod.soundRange,
									player.dimension, offHandGunType.cooldownSound, false);
							data.shouldPlayCooldownSound = false;
						}
					}
				}
	...
	public void onUpdate(ItemStack itemstack, World world, Entity pEnt, int i, boolean flag) {
		...
		if (pEnt instanceof EntityPlayer) {
			...
			// data.lastMeleePositions.length != type.meleeDamagePoints.size())
			// {
			// data.lastMeleePositions = new
			// Vector3f[type.meleeDamagePoints.size()];
			// for(int j = 0; j < type.meleeDamagePoints.size(); j++)
			// data.lastMeleePositions[j] = new Vector3f(player.posX,
			...
			if (data.meleeLength > 0 && type.meleePath.size() > 0 && player.inventory.getCurrentItem() == itemstack) {
				for (int k = 0; k < type.meleeDamagePoints.size(); k++) {
					...
					// Do a raytrace from the prev pos to the current pos and
					// attack anything in the way
					Vector3f nextPos = type.meleePath.get((data.meleeProgress + 1) % type.meleePath.size());
					Vector3f nextAngles = type.meleePathAngles
							.get((data.meleeProgress + 1) % type.meleePathAngles.size());
					RotatedAxes nextAxes = new RotatedAxes().rotateGlobalRoll(-nextAngles.x)
							.rotateGlobalPitch(-nextAngles.z).rotateGlobalYaw(-nextAngles.y);
					...
					Vector3f nextPosInPlayerCoords = new RotatedAxes(player.rotationYaw + 90F, player.rotationPitch, 0F)
							.findLocalVectorGlobally(nextPosInGunCoords);

					if (!FlansMod.proxy.isThePlayer(player))
						nextPosInPlayerCoords.y += 1.6F;

					Vector3f nextPosInWorldCoords = new Vector3f(player.posX + nextPosInPlayerCoords.x,
							player.posY + nextPosInPlayerCoords.y, player.posZ + nextPosInPlayerCoords.z);

					Vector3f dPos = data.lastMeleePositions[k] == null ? new Vector3f()
							: Vector3f.sub(nextPosInWorldCoords, data.lastMeleePositions[k], null);

					if (player.worldObj.isRemote && FlansMod.DEBUG)
						player.worldObj.spawnEntityInWorld(new EntityDebugVector(player.worldObj,
								data.lastMeleePositions[k], dPos, 200, 1F, 0F, 0F));
						...
						for (int j = 0; j < world.loadedEntityList.size(); j++) {
							...
							if (obj instanceof EntityPlayer) {
								...
								if (otherData != null) {
									...
									int snapshotToTry = player instanceof EntityPlayerMP
											? ((EntityPlayerMP) player).ping / 50 : 0;
									...
									// Check one last time for a null snapshot.
									// If this is the case, fall back to normal
									// hit detection
									if (snapshot == null)
										shouldDoNormalHitDetect = true;
									else {
										// Raytrace
										ArrayList<BulletHit> playerHits = snapshot
												.raytrace(data.lastMeleePositions[k] == null ? nextPosInWorldCoords
														: data.lastMeleePositions[k], dPos);
										hits.addAll(playerHits);
									}
								}

								// If we couldn't get a snapshot, use normal
								// entity hitbox calculations
								if (otherData == null || shouldDoNormalHitDetect) {
									MovingObjectPosition mop = data.lastMeleePositions[k] == null
											? player.boundingBox.calculateIntercept(nextPosInWorldCoords.toVec3(),
													Vec3.createVectorHelper(0F, 0F, 0F))
											: player.boundingBox.calculateIntercept(data.lastMeleePositions[k].toVec3(),
													nextPosInWorldCoords.toVec3());
									if (mop != null) {
										...
										hits.add(new PlayerBulletHit(
												new PlayerHitbox(otherPlayer, new RotatedAxes(), new Vector3f(),
														new Vector3f(), new Vector3f(), EnumHitboxType.BODY),
												hitLambda));
									}
								}
							} else {
						...
						if (!hits.isEmpty()) {
							...
							for (BulletHit bulletHit : hits) {
								if (bulletHit instanceof PlayerBulletHit) {
									...
									case LEFTITEM:
									case RIGHTITEM: // Hit a shield. Stop the
													// swing.
									{
									...
									if (playerHit.hitbox.player.attackEntityFrom(getMeleeDamage(player),
										...
										// If the attack was allowed, we should
										// remove their immortality cooldown so
										// we can shoot them again. Without
										// this, any rapid fire gun become
										// useless
										playerHit.hitbox.player.arrowHitTimer++;
										playerHit.hitbox.player.hurtResistantTime = playerHit.hitbox.player.maxHurtResistantTime
												/ 2;
									}

									if (FlansMod.DEBUG)
										world.spawnEntityInWorld(new EntityDebugDot(world,
												new Vector3f(data.lastMeleePositions[k].x
														+ dPos.x * playerHit.intersectTime,
												data.lastMeleePositions[k].y + dPos.y * playerHit.intersectTime,
												data.lastMeleePositions[k].z + dPos.z * playerHit.intersectTime), 1000,
												1F, 0F, 0F));
									...
									if (FlansMod.DEBUG)
										world.spawnEntityInWorld(new EntityDebugDot(world,
												new Vector3f(data.lastMeleePositions[k].x
														+ dPos.x * entityHit.intersectTime,
												data.lastMeleePositions[k].y + dPos.y * entityHit.intersectTime,
												data.lastMeleePositions[k].z + dPos.z * entityHit.intersectTime), 1000,
												1F, 0F, 0F));
								}
							}
						}
		...
		if (data != null && data.shootClickDelay == 0) {
			...
			if (left && data.offHandGunSlot != 0) {
				...
				if (gunType.getFireMode(offHandGunStack) == EnumFireMode.SEMIAUTO && isShooting) {
					...
					player.inventory.setInventorySlotContents(data.offHandGunSlot - 1,
							tryToShoot(offHandGunStack, gunType, world, player, true));
				}
				if (gunType.getFireMode(offHandGunStack) == EnumFireMode.BURST && isShooting
						&& data.burstRoundsRemainingLeft == 0) {
					data.isShootingLeft = false;
					data.burstRoundsRemainingLeft = gunType.numBurstRounds;
					player.inventory.setInventorySlotContents(data.offHandGunSlot - 1,
				...
				if (type.getFireMode(stack) == EnumFireMode.SEMIAUTO && isShooting) {
					...
					player.inventory.setInventorySlotContents(player.inventory.currentItem,
							tryToShoot(stack, type, world, player, false));
				}
				if (type.getFireMode(stack) == EnumFireMode.BURST && isShooting
	...
	public ItemStack tryToShoot(ItemStack gunStack, GunType gunType, World world, EntityPlayerMP entityplayer,
		...
		if ((left && data.shootTimeLeft <= 0) || (!left && data.shootTimeRight <= 0)) {
			// Go through the bullet stacks in the gun and see if any of them
			// are not null
			int bulletID = 0;
			ItemStack bulletStack = null;
			for (; bulletID < gunType.numAmmoItemsInGun; bulletID++) {
				ItemStack checkingStack = getBulletItemStack(gunStack, bulletID);
				if (checkingStack != null && checkingStack.getItem() != null
						&& checkingStack.getItemDamage() < checkingStack.getMaxDamage()) {
				...
				if (reload(gunStack, gunType, world, entityplayer, false, left)) {
					...
					if (gunType.reloadSound != null)
						PacketPlaySound.sendSoundPacket(entityplayer.posX, entityplayer.posY, entityplayer.posZ,
								FlansMod.soundRange, entityplayer.dimension, gunType.reloadSound, true);
				}
			}
	...
	public boolean reload(ItemStack gunStack, GunType gunType, World world, EntityPlayer player, boolean forceReload,
			boolean left) {
		return reload(gunStack, gunType, world, player, player.inventory, player.capabilities.isCreativeMode,
				forceReload);
	}
	...
	public boolean reload(ItemStack gunStack, GunType gunType, World world, Entity entity, IInventory inventory,
		...
		if (gunType.deployable)
			return false;
		// If you cannot reload half way through a clip, reject the player for
		// trying to do so
		...
		for (int i = 0; i < gunType.numAmmoItemsInGun; i++) {
			...
			// If there is no magazine, if the magazine is empty or if this is a
			// forced reload
			if (bulletStack == null || bulletStack.getItemDamage() == bulletStack.getMaxDamage() || forceReload) {
				// Iterate over all inventory slots and find the magazine /
				// bullet item with the most bullets
				int bestSlot = -1;
				int bulletsInBestSlot = 0;
				for (int j = 0; j < inventory.getSizeInventory(); j++) {
					ItemStack item = inventory.getStackInSlot(j);
					if (item != null && item.getItem() instanceof ItemShootable
							&& gunType.isAmmo(((ItemShootable) (item.getItem())).type)) {
				...
				// If there was a valid non-empty magazine / bullet item
				// somewhere in the inventory, load it
				if (bestSlot != -1) {
					...
					// Unload the old magazine (Drop an item if it is required
					// and the player is not in creative mode)
					...
					// The magazine was not finished, pull it out and give it
					// back to the player or, failing that, drop it
	...
	private void shoot(ItemStack stack, GunType gunType, World world, ItemStack bulletStack, EntityPlayer entityplayer,
			boolean left) {
		...
		if (soundDelay <= 0 && gunType.shootSound != null) {
			...
			// world.playSoundAtEntity(entityplayer, type.shootSound, 10F,
			// type.distortSound ? 1.0F / (world.rand.nextFloat() * 0.4F + 0.8F)
			// : 1.0F);
			PacketPlaySound.sendSoundPacket(entityplayer.posX, entityplayer.posY, entityplayer.posZ,
					FlansMod.soundRange, entityplayer.dimension, gunType.shootSound, gunType.distortSound, silenced);
		...
		if (!world.isRemote) {
			...
			for (int k = 0; k < gunType.numBullets; k++) {
				world.spawnEntityInWorld(((ItemShootable) bulletStack.getItem()).getEntity(world, entityplayer,
						(entityplayer.isSneaking() ? 0.7F : 1F) * gunType.getSpread(stack), gunType.getDamage(stack),
						gunType.getBulletSpeed(stack), gunType.numBullets > 1, bulletStack.getItemDamage(), gunType));
			}
	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer) {
		if (type.deployable) {
			...
			double length = 5D;
			Vec3 posVec = Vec3.createVectorHelper(entityplayer.posX, entityplayer.posY + 1.62D - entityplayer.yOffset,
					entityplayer.posZ);
								...
								&& (world
										.getBlock(
												i + (playerDir == 1 ? 1 : 0) - (playerDir == 3 ? 1 : 0), j
														+ 1,
												k - (playerDir == 0 ? 1 : 0) + (playerDir == 2 ? 1 : 0)) == Blocks.air)
								&& (world.getBlock(i + (playerDir == 1 ? 1 : 0) - (playerDir == 3 ? 1 : 0), j,
										k - (playerDir == 0 ? 1 : 0) + (playerDir == 2 ? 1 : 0)) == Blocks.air
										|| world.getBlock(i + (playerDir == 1 ? 1 : 0) - (playerDir == 3 ? 1 : 0), j,
												k - (playerDir == 0 ? 1 : 0)
														+ (playerDir == 2 ? 1 : 0)) == Blocks.snow)) {
		...
		// Stop the gun bobbing up and down when holding shoot and looking at a
		// block
		if (world.isRemote) {
			for (int i = 0; i < 3; i++)
	...
	public boolean onEntitySwing(EntityLivingBase entityLiving, ItemStack stack) {
		if (type.meleeSound != null)
			PacketPlaySound.sendSoundPacket(entityLiving.posX, entityLiving.posY, entityLiving.posZ,
					FlansMod.soundRange, entityLiving.dimension, type.meleeSound, true);
	...
	public Multimap getAttributeModifiers(ItemStack stack) {
		...
		map.put(SharedMonsterAttributes.movementSpeed.getAttributeUnlocalizedName(),
				new AttributeModifier(field_111210_e, "MovementSpeed", type.moveSpeedModifier - 1F, 2));
