...
public class GuiTeamScores extends GuiScreen 
{
	...
	public void renderTwoTeamGUI(PacketTeamInfo teamInfo)
	{
		...
		if(teamInfo.roundOver())
		{
			...
			if(winners == null)
			{
				drawString(fontRendererObj, "Temps f.ec !", m + 10, n + 20, 0xffffff);
			}
			else
			{
				drawString(fontRendererObj, winners.name + " f.g !", m + 10, n + 20, 0xffffff);
			}
		}
		else
		{
			...
			drawString(fontRendererObj, "Temps restant : " + minutesLeft + ":" + (secondsLeft < 10 ? "0" + secondsLeft : secondsLeft), m + 10, n + 20, 0xffffff);
			drawString(fontRendererObj, "Limite de scores : " + teamInfo.scoreLimit, m + 302 - fontRendererObj.getStringWidth("Limite de scores : " + teamInfo.scoreLimit), n + 20, 0xffffff);
		}
	...
	public void renderDMGUI(PacketTeamInfo teamInfo)
	{
		...
		drawString(fontRendererObj, "Nom", m + 8, n + 14, 0xffffff);
		drawString(fontRendererObj, "Score", m + 100, n + 14, 0xffffff);
		drawString(fontRendererObj, "f.tu", m + 150, n + 14, 0xffffff);
		drawString(fontRendererObj, "Morts", m + 200, n + 14, 0xffffff);
		int line = 0;
		if(teamInfo.sortedByTeam)
		{
			for(int p = 0; p < teamInfo.numTeams; p++)
